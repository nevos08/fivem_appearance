...
`nvx_appearance` solves this with a statebag. It is on by default and can be turned off.

## How it works

1. A client applies an appearance locally and pushes it to the server.
2. The server normalizes it and writes it to `Player(src).state['nvx_appearance:data']`, replicated.
...
```lua
local hair = exports.nvx_appearance:GetHair(src)   -- returns immediately
```
...
CreateThread(function()
    local hair, reason = exports.nvx_appearance:GetHair(src)
    if not hair then
...
So `exports.nvx_appearance:SetComponent(src, 'torso', '', 9999, 0)` returns `true` from the server (the message was sent) while the client clamps or rejects the value. If you need the outcome, read the value back.
...
* `nvx_appearance:appearanceChanged` no longer fires.
