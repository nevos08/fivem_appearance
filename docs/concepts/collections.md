...
Every clothing slot in `nvx_appearance` carries a `collection` alongside its `drawable`. This is the single most important difference from older skin systems, and the reason saved outfits do not break.
...
`nvx_appearance` uses the collection form everywhere and stores the local index.
...
```lua
exports.nvx_appearance:SetComponent('torso', '', 15, 0)                  -- base game
exports.nvx_appearance:SetComponent('pants', 'female_heist', 9, 3)       -- DLC
```
...
local collections = exports.nvx_appearance:GetCollections()

-- how much is in one of them
local drawables, textures = exports.nvx_appearance:GetVariationCounts('torso', 'female_heist', 4)
...
```lua
local global = exports.nvx_appearance:ToGlobalIndex('torso', '', 15)
local collection, localIndex = exports.nvx_appearance:FromGlobalIndex('torso', global)
```
...
If a component or prop arrives **without** a `collection` field, `nvx_appearance` treats its `drawable` as a legacy global index and converts it on apply.
