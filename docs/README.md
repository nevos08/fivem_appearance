...
`nvx_appearance` is the layer that applies and reads a ped's appearance. It is a **backend only** - there is no UI and no character creator. You drive it entirely through exports, from both the client and the server.
...
It also does not replace `esx_skin`: that one additionally ships a clothing menu and persists to `users.skin`. `nvx_appearance` does neither, so you keep your own UI and your own persistence - see [Quickstart](getting-started/quickstart.md).
{% endhint %}

## How it compares to skinchanger

| | skinchanger | nvx_appearance |
| --- | --- | --- |
...
**No persistence.** `nvx_appearance` never touches a database. It holds the current appearance in memory and, if sync is on, in a statebag. Saving and loading is your resource's job - see [Quickstart](getting-started/quickstart.md).

**Exports only.** Events exist purely as outbound hooks, never as a control path. There is no `TriggerEvent('nvx_appearance:loadSkin', …)`.
...
```lua
-- client
exports.nvx_appearance:SetComponent('torso', '', 15, 0)
local hair = exports.nvx_appearance:GetHair()

-- server
exports.nvx_appearance:SetComponent(source, 'torso', '', 15, 0)
local appearance = exports.nvx_appearance:GetAppearance(source)
```
